...
export const fetchProducts = async (setProducts) => {
    try {
        const response = await axios.get(
            "http://localhost:81/php_lab/api/products"
        );
...
export const deleteProduct = async (id, setProducts, products) => {
    try {
        await axios.delete(`http://localhost:81/php_lab/api/products/${id}`);
...
export const fetchProductDetails = async (
    ...
    try {
        const response = await axios.get(
            `http://localhost:81/php_lab/api/products/${id}`
        );
        ...
        if (!setCategories || setCategories.length === 0) {
            const categoriesResponse = await axios.get(
                "http://localhost:81/php_lab/api/categories"
            );
...
export const getProductByCategory = async (category) => {
    try {
        const response = await axios.get(
            `http://localhost:81/php_lab/api/products?category=${category}`
        );
        return response.data;
    } catch (error) {
        console.error("Error fetching product:", error);
