...
export const fetchOrders = async (setOrders, notification) => {
    try {
        const response = await axios.get(
            "http://localhost:81/php_lab/api/orders"
        );
...
export const fetchOrderDetails = async (
    ...
    try {
        const response = await axios.get(
            `http://localhost:81/php_lab/api/detail/${id}`
        );
...
export const submitOrder = async (
    ...
    try {
        const response = await axios.post(
            `http://localhost:81/php_lab/api/submit`,
            {
...
export const getPaymentSuccessMessage = async () => {
    try {
        const response = await axios.get(
            `http://localhost:81/php_lab/api/orders/confirmation`
        );
